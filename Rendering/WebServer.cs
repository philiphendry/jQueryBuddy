...
namespace jQueryBuddy.Rendering
{
    ...
    public sealed class TestWebServer : IDisposable
    {
        ...
        public string ExtractResource(string resourceName, string destinationFileName)
        {
            ...
            {
                filePath = WritePageContents(destinationFileName, stream);
            }
            return filePath;
        }

        private string WritePageContents(string destinationFileName, Stream stream)
        {
            string filePath;
        ...
        public void WritePage(string pageFilename, string pageContents)
        {
            using (var ms = new MemoryStream())
            {
                using (var sw = new StreamWriter(ms))
                {
                    sw.Write(pageContents);
                    sw.Flush();
                    ms.Seek(0, SeekOrigin.Begin);
                    WritePageContents(pageFilename, ms);
                }
            }
        }
        ...
        private void ReleaseManagedResources()
        {
            ...
            {
                // Need to figure out why the webRoot folder is locked before
                // removing this error handler that swallows any exceptions.
            }
        }
    }
}
